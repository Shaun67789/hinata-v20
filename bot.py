...
async def global_neural_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat and chat.type in ["group", "supergroup"]:
        ...
        if len(GROUP_MEMBERS[cid]) > 100:
            # Simple way to pop oldest or just keep it manageable
...
def load_config():
    return read_json(CONFIG_FILE, {
        "global_access": True, 
        "banned_users": [],
        "couple_bg": "https://graph.org/file/cf7f0fc66d178f2528f4a-c9dcd2c28ae462aac9.jpg"
    })
...
async def cmd_promote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_id:
        await update.effective_message.reply_text(" <b>Usage:</b> Reply to a message or mention them to `/promote`.", parse_mode="HTML")
        return
    ...
    try:
        await context.bot.promote_chat_member(
            ...
            can_manage_video_chats=True,
            can_restrict_members=True,
            can_promote_members=False, # Restricted from adding other admins
            can_change_info=True,
            can_invite_users=True,
            can_pin_messages=True
        )
        await update.effective_message.reply_text(f"👑 User has been promoted to Admin.")
    except Exception as e:
